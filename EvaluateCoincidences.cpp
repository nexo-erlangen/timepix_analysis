...
void ScatterPlotEnergies3CoincidenceGroups(std::string filename){
    int countConversion =0;
    f = new TFile(filename.c_str(), "OPEN");
    ...
    t->GetBranch("coincidence_group_size")->SetAddress(&coinc_group_size);
    t->GetBranch("clstrSize")->SetAddress(&clstrSize);
    t->GetBranch("min_ToA")->SetAddress(&clstrTime);
    t->GetBranch("ToT_keV")->SetAddress(ToT_keV);
    t->GetBranch("PixX")->SetAddress(PixX);
    t->GetBranch("PixY")->SetAddress(PixY);
    
    
    //Create and entry list of interesting events;
    t->Draw(">>myList", "coincidence_group_size == 3", "entryList");
    ...
    TH1F* h_e = new TH1F("h_e", "", 1000, 0, 1000);
    TH2F* pix = new TH2F("pixelmatrix", "", 256, -0.5,255.5,256,-0.5,255.5);
    ...
    for(long i = 0; i < myList->GetN(); i++){
        ...
        if(coinc_group_size==3)
        {
            for(long k = 0; k < 3; k++)
            {
                t -> GetEntry(myList->GetEntry(i+k));
                energies[k] = clstrVolume_keV;
                size[k] = clstrSize;
                time[k] = clstrTime;
            }
             ...
             */
            if((size[0] == 1 && size[1] == 1 && size[2] > 2 && energies[0] <10  && energies[1] < 10 && energies[2] > 750 && energies[2] < 900) || (size[1] == 1 && size[2] == 1 && size[0] > 2 && energies[1] <10  && energies[2] < 10 && energies[0] > 750 && energies[0] < 900) || (size[2] == 1 && size[0] == 1 && size[1] > 2 && energies[2] <10  && energies[0] < 10 && energies[1] > 750 && energies[1] < 900) ){
                // h_e1e2->Fill(energies[0], energies[1]);
                h_dt->Fill(fabs(time[2] - time[0]));
                for (int m = 0; m < 3; m++) {
                    if (energies[m]>600) {
                        h_e->Fill(energies[m]);
                        for (int n=0; n < size[m]; n++) {
                            pix->Fill(PixX[n],PixY[n],ToT_keV[n]);
                       }
                    }
                }
...
c[0]->cd();
defineHistogram(h_dt, "#Deltat (ns)", "Number of events", "Coinc. Group Size: 3, cluster size <= 2", false);
h_dt->GetXaxis()->SetTitleOffset(0.9);
h_dt->GetYaxis()->SetTitleOffset(0.9);
h_dt->Draw("");

c[1]->cd();
defineHistogram(h_e, "#SigmaE (keV)", "Number of events", "Coinc. Group Size: 3, cluster size <= 2", false);
h_e->GetXaxis()->SetTitleOffset(0.9);
h_e->GetYaxis()->SetTitleOffset(0.9);
h_e->Draw("");

c[2]->cd();
defineHistogram(pix, "#ToT (keV)", "Number of events", "Coinc. Group Size: 3, cluster size > 2", false);
pix->GetXaxis()->SetTitleOffset(0.9);
pix->GetYaxis()->SetTitleOffset(0.9);
pix->Draw("COLZ9");
